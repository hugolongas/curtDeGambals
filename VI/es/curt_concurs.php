<?php
	...
	<?php for($i=0;$i<$totalConcurs;$i++){
		...
		$sinopsis = mysql_result($concurs,$i,'sinopsis_cas');
	...
	<?php $obert=1;$pag++;}?>
			...
			<div id="curt<?php echo $i; ?>" class='curt <?php if($votat[$id-1]=="1"){echo " votat";} ?>'>
				...
				<span class="vots">votos: <?php echo $vots ?></span>
	...
	<script>
		$(".curt").click(function(){
			...
			if(!$("#"+curt).hasClass("votat")){
				...
				$("#votacio", window.parent.document).html("<span class='novotar'>Ya has votado este corto hoy, vuelve en 24H</span>");
			}
			$("#votacio", window.parent.document).html("<span class='novotar'>Ya ha finalizado el concurso web, Gracias a todos por participar</span>");
